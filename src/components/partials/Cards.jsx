...
const Card = ({ data }) => {
  ...
  return (
    <Link to={`/${media_type}/${id}`} className="md:w-60 hover:scale-105 flex-shrink-0 w-full m-4 transition-transform duration-500 transform">
        ...
        <div className="p-4 bg-black bg-opacity-50">
          ...
          <div className="flex gap-2 mt-2">
            <p className="text-sm text-gray-300">
              {media_type ? (
                media_type === 'movie' ? (
                  <BsFilm className="mr-1 text-yellow-500" />
                ) : (
                  <BsTv className="mr-1 text-red-500" />
                )
              ) : (
                <></>
              )}
